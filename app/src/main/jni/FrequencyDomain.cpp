...
Java_com_example_light_CPPActivity_FrequencyDomain(JNIEnv * __unused env, jobject __unused obj, jint samplerate, jint buffersize) {
...
extern "C" JNIEXPORT void
Java_com_example_light_CPPActivity_Cleanup(JNIEnv * __unused env, jobject __unused obj) {
    delete audioIO;
    free(magnitudeLeft);
    free(magnitudeRight);
    free(phaseLeft);
...
//};
//
//// Function to get the closest note and its frequency
//void getClosestNote(float frequency, const char*& note, float& noteFrequency) {
//    int noteNumber = (int)round(12.0 * log2(frequency / 440.0)) + 69;
//    noteNumber = noteNumber % 12; // Limit to one octave (C to B)
//    note = noteNames[noteNumber];
//    noteFrequency = 440.0 * pow(2.0, (noteNumber - 69) / 12.0);
//}
//
//static Superpowered::FrequencyDomain *frequencyDomain;
//static SuperpoweredAndroidAudioIO *audioIO;
//static float *magnitudeLeft, *magnitudeRight, *phaseLeft, *phaseRight, *fifoOutput, *inputBufferFloat;
//
//bool audioProcessing(void * __unused clientdata, short int *audioInputOutput, int numberOfFrames, int samplerate) {
//    Superpowered::ShortIntToFloat(audioInputOutput, inputBufferFloat, (unsigned int)numberOfFrames);
//
//    // Input goes to the frequency domain.
//    frequencyDomain->addInput(inputBufferFloat, numberOfFrames);
//
//    // Example pitch detection logic
//    float pitch = frequencyDomain->getFrequency();
//
//    const char* note;
//    float noteFrequency;
//    getClosestNote(pitch, note, noteFrequency);
//
//    __android_log_print(ANDROID_LOG_DEBUG, "Superpowered", "Detected pitch: %f Hz, Note: %s, Note frequency: %f Hz", pitch, note, noteFrequency);
//
//    return true;
//}
//
//extern "C" JNIEXPORT void JNICALL
//Java_com_example_light_CPPActivity_FrequencyDomain(JNIEnv *env, jobject instance) {
//    frequencyDomain = new SuperpoweredFrequencyDomain(44100);
//    audioIO = new SuperpoweredAndroidAudioIO(44100, 512, false, true, audioProcessing, NULL, -1, SL_ANDROID_STREAM_MEDIA);
//}
//
//extern "C" JNIEXPORT void JNICALL
//Java_com_example_light_CPPActivity_StopFrequencyDomain(JNIEnv *env, jobject instance) {
//    delete audioIO;
